...
if(global.should_dia[dia_id][1]){ //end
	dialogue_obj = instance_create_layer(x,y,"Instances",oDialog0TutorialEnd);
	dialogue_obj.dia_id = dia_id;
	
	dialogue_obj = instance_create_layer(0,0,"Instances",oDialog0TutorialStart);
	dialogue_obj.image_xscale = 0.1;
	dialogue_obj.image_yscale = 0.1;
	dialogue_obj.dia_id = dia_id;
	y-=32;
} else if(global.should_dia[dia_id][0]){ //start
	dialogue_obj = instance_create_layer(x,y+16,"Instances",oDialog0TutorialStart);
	dialogue_obj.dia_id = dia_id;
	dialogue_obj.dia_id = dia_id;
} else{
	dialogue_obj = instance_create_layer(0,0,"Instances",oDialog0TutorialStart);
